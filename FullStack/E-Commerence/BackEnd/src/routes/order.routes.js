    ...
    try {
        ...
        for (const cartItem of cart.products) {
            ...
            if (product.stockQuantity < cartItem.quantity) {
                return res.status(400).json(`Not enough stock for ${product.name}. Available: ${product.stockQuantity}`);
            }
...
router.get('/:id', verifyToken, async (req, res) => { // Using verifyToken here, check ownership inside
    try {
